...
% Evaluate kickback energies.
hamPar.E_TRx_EXP = 0.5*par.mTOT*vCOGx(end)^2;
hamPar.E_TRy_EXP = 0.5*par.mTOT*vCOGy(end)^2;
hamPar.E_ROT_EXP = 0.5*par.JTOT*omCOGz(end)^2;

% Calculation with Hand-Arm-System described in the ANSI norm for chainsaws;
% no external forces but initial velocities taken from previous calculation
% without HAM.
...
vCOGx0 = vCOGx(end);     % x-velocity COG
vCOGy0 = vCOGy(end);     % y-velocity COG
omCOGz0 = omCOGz(end);    % angular velocity about axis through COG
...
% We have only the force on SCH here.
forceFcn = @(t, y) (fHAM(t, y, par, hamPar));
